				...
				<div id="uno">
					<table border="1" width="900">
						...
						<?php
							...
							$i=0;
							$lung=count($nomi);
							echo "<form action='togli_articoli.php?nomeUtente=$nomeUtente' method='post'>";
							while($i < $lung) {
							   $id_stringa=(string)$id_prodotti[$i];
							   $link="Prodotti/".$id_stringa.".jpg";
							   echo "<tr><td><input type='checkbox' name='prendi[]' value='$date[$i]&$id_prodotti[$i]'></td><td>$id_prodotti[$i]</td><td>$nomi[$i]</td><td>$prezzi[$i]</td><td>$colori[$i]</td><td>$categorie[$i]</td><td>$date[$i]</td><td><a href='$link'><img src=$link width='100' height='150'></a></td><tr>";
							   $i++;
							}
							echo "<input type='submit' value='Togli'></form>"
						?>
					</table>
				</div>
			</div><!--#main-->
